...
Swift SDK for the BSV Blockchain (`BSV` Swift Package). Part of the official BSV SDK family alongside [ts-sdk](https://github.com/bsv-blockchain/ts-sdk), [go-sdk](https://github.com/bsv-blockchain/go-sdk), and [py-sdk](https://github.com/bsv-blockchain/py-sdk).

**Reference priority when implementing features:**

1. **[ts-sdk](https://github.com/bsv-blockchain/ts-sdk) — the primary reference.** Its architecture, naming, and API shape are what we follow. When in doubt, match ts-sdk.
2. **Additional references** (for cross-checking algorithms and test vectors):
   - [go-sdk](https://github.com/bsv-blockchain/go-sdk) — canonical test vectors live here (script tests, sighash, BRC-42)
   - [py-sdk](https://github.com/bsv-blockchain/py-sdk) — useful for comparison, but wraps libsecp256k1 so diverges on crypto
   - [sgbett/bsv-ruby-sdk](https://github.com/sgbett/bsv-ruby-sdk) — a sibling Ruby implementation, useful as a familiar comparison point

**Reference SDK clones** are at `/opt/ruby/bsv-reference-sdks/` (`ts-sdk/`, `go-sdk/`, `py-sdk/`). The Ruby SDK sibling is at `/opt/ruby/bsv-ruby-sdk`. Run `git -C <path> pull` to update before comparing.
